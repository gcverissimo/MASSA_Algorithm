...
* **Input file**: ```-i``` or ```--input```.
    * MASSA Algorithm accepts input files in the formats: .sdf, .mol, .mol2, .xlsx, .xls, .csv, and .smi. However, the .sdf format is preferred. Notes:
        ...
        * For .xlsx, .xls, .smi, and .csv:
            ...
            * For .smi files, the first line must contain column headers.
