...
def capture_args():
    ...
    parser = argparse.ArgumentParser(
        ...
        usage=(
            ...
            '[-x <frequency_bar_plot_font_size_on_Xaxis>] '
            '[-p <percentage_of_molecules_in_training_set>] '
            '[-b <number_of_biological_activities_in_input_file>] '
            '[-s <activity1>,<activity2>,<activityN>] [-n <number_of_principal_components>] '
            '[-v <svd_parameter>] [-l <linkage_method]  [-f <toggle_dendrogram>] '
            '[-y <splitting_strategy>]'
        )
    )
