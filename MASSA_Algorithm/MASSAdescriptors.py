...
def physicochemical_descriptors(file):
    ...
    for i in file['molecules']:
        NumHAcceptors[i.GetProp('_Name')] = Descriptors.NumHAcceptors(
            i)  # CALC: number of hydrogen acceptors
        NumHDonors[i.GetProp('_Name')] = Descriptors.NumHDonors(
            i)  # CALC: number of hydrogen donors
        ExactMolWt[i.GetProp('_Name')] = Descriptors.ExactMolWt(
            i)  # CALC: molecular weight
        NumRotatableBonds[i.GetProp('_Name')] = Descriptors.NumRotatableBonds(
            i)  # CALC: number of rotatable bounds
        FractionCSP3[i.GetProp('_Name')] = Descriptors.FractionCSP3(
            i)  # CALC: sp3 carbon fraction
        TPSA[i.GetProp('_Name')] = Descriptors.TPSA(i)  # CALC: TPSA
        LogP_WildmanCrippen[i.GetProp('_Name')] = Descriptors.MolLogP(
            i)  # CALC: WildmanCrippen Log P
...
def atompairs_fingerprint(file):
    ...
    for i in file['molecules']:
        ...
        # Create a "molecule name":fingerprint dictionary.
